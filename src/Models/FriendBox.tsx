...
const FriendBox = ({ setaddFriendShow }: FriendBoxType) => {
    ...
    return (
        <div className='fixed top-[100dvh] min-h-[60%] max-h-[85%] w-[25vw] sm:w-[75vw] flex flex-col items-center shadow-lg bg-[#ffffffb0] sm:backdrop-blur-md sm:bg-[#ffffffb2] dark:bg-[#1e1a2fbd] sm:shadow-lg sm:rounded-lg sm:p-4 z-[100]' ref={friendElement}>
            <p className="absolute right-4 top-5 sm:right-1 sm:top-1  invert-[0.4] opacity-70 dark:opacity-100 dark:invert-0 cursor-pointer select-none p-2 text-xs rounded-full" onClick={() => { setaddFriendShow(false) }}>❌</p>
            <div className="flex flex-row items-center py-2">
                <input type="text" placeholder="Search Friends" className=" pl-[2vw] sm:pl-8 w-full border-none focus:outline-none rounded-md h-[2vw] sm:h-10 bg-white dark:bg-slate-600 dark:text-slate-300 text-[0.8vw] sm:text-xs text-gray-800" defaultValue={inputValue} onChange={(e)=>{setInputValue(e.currentTarget.value)}}/>
                <i className="fi fi-sr-woman-head absolute left-[0.3vw] dark:text-slate-950 sm:left-2 text-gray-600 mt-1"></i>
            </div>
                ...
                return <div className="flex flex-row items-center px-4 py-3 bg-white dark:bg-slate-700 dark:text-slate-300 my-[0.5vw] w-full rounded-lg">
                    {
                     ...
                     <h2 className="text-[1vw] font-medium pl-2 sm:text-sm">{each.username}</h2>
                    {
                        friendAlreadyReqSentCheck(each.uid) ?
                        <i className="fi fi-sr-user-add ml-auto rounded-full flex items-center justify-center  h-[1.8vw] w-[1.8vw] sm:w-5 sm:h-5 text-[0.7vw] sm:text-xs cursor-pointer bg-pink-50 text-slate-400"><span className="absolute -left-[1vw]">✔️</span></i>
                        :
                        <i className="fi fi-sr-user-add ml-auto rounded-full flex items-center justify-center hover:text-white hover:bg-pink-700 h-[1.8vw] sm:h-6 sm:w-6 w-[1.8vw] text-[0.7vw] sm:text-xs cursor-pointer bg-pink-100 text-pink-800" onClick={() => { friendReq({ 'uid': each.uid, 'username': each.username }) }}></i>
                    }
                </div>
            })}
        </div>
